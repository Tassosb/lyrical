...
var WordCounter = __webpack_require__(31);
var StreamGraph = __webpack_require__(29);
var InputValidator = __webpack_require__(27);
...
var App = function () {
  function App() {
    ...
    axios.get('http://localhost:3000/counts').then(function (res) {
      _this.setup(res.data);
    });
  }

  _createClass(App, [{
    key: 'setup',
    value: function setup(lyricsData) {
    ...
    value: function handleInputChange(e) {
      ...
      var exists = this.validator.validate(text);
      if (!exists) {
    ...
    value: function handleSubmit(e) {
      e.preventDefault();
      var counts = this.counter.count(this.input.value, true);
      this.graph.setData(counts);

      var message = '"' + this.counter.targets.join("\", \"") + '"';
      document.querySelector("strong.search-terms").innerText = message;
    }
  }, {
    key: 'togglePercent',
    value: function togglePercent(bool) {
      var _this2 = this;

      return function (e) {
        _this2.counter.percent = bool;
        _this2.graph.props.percent = bool;
        if (bool) {
          _this2.percentEl.classList.add('selected');
          _this2.countEl.classList.remove('selected');
        } else {
          _this2.percentEl.classList.remove('selected');
          _this2.countEl.classList.add('selected');
        }
        _this2.graph.setData(_this2.counter.count());
      };
    }
  }, {
    key: 'handleToggleInclude',
    value: function handleToggleInclude(e) {
      e.preventDefault();
...
var _require = __webpack_require__(30),
    Trie = _require.Trie,
    TrieNode = _require.TrieNode;

var InputValidator = function () {
  function InputValidator(lyricsData) {
  ...
  _createClass(InputValidator, [{
    ...
    value: function buildTrie() {
      ...
      this.lyricsData.forEach(function (song) {
        var words = Object.keys(song.Lyrics);
        words.forEach(function (word) {
          var chars = word.split('');
          var currNode = _this.root;
...
var StreamGraph = function () {
  ...
  _createClass(StreamGraph, [{
    ...
    value: function draw() {
      ...
      this.plot = this.svg.append('g').attr('transform', 'translate(0,' + this.margin.top + ')');
    ...
    value: function drawPaths() {
      ...
      this.svg.selectAll("path").data(this.layers).enter().append("path").attr("d", this.area).attr("fill", function () {
        return z(Math.random());
      }).on('mouseover', function (d) {
        popUp.style('opacity', 1);
        [sightLine, sightY].forEach(function (el) {
        ...
        if (isPercent) {
          count = count < 0.00001 ? "< .00001" : Math.round(count * 10000) / 10000;
        } else {
        ...
        popUp.style('left', mousex + 2 + 'px').html(StreamGraph.createPopUpHTML(popUpOptions));
    ...
    value: function transition(data) {
      d3.selectAll("path").data(this.layers).transition().duration(2500).attr("d", this.area);
    }
  }, {
    key: 'addAxes',
    value: function addAxes() {
      ...
      var m = this.margin;
      this.plot.append("g").attr("class", "x axis").attr("transform", "translate(" + 0 + "," + (this.height - (m.top + m.bottom)) + ")").call(this.xAxis);

      this.plot.append("g").attr("class", "y axis").attr("transform", "translate(" + m.left + "," + (-m.bottom - 8) + ")").call(this.yAxis);

      this.plot.append("text").attr("class", "y axis label").attr("x", 0).attr("y", (this.height - (m.top + m.bottom)) / 4).text(this.props.percent ? "%" : "#");

      this.plot.append("text").attr("class", "x axis label").attr("x", (this.width - m.right) / 2).attr("y", this.height - 25).text("year");
    }
  }, {
    key: 'createLayers',
    value: function createLayers() {
      ...
      var layerData = d3.transpose(d3.range(StreamGraph.NUM_LAYERS).map(function () {
        return _this.data;
      }));
    ...
    value: function getYears() {
      if (this.years === undefined) this.years = this.data.map(function (d) {
        return d.year;
      });
      return this.years;
    }
    ...
    value: function createPopUp() {
      this.popUp = d3.select('#graph').append('div').attr('class', 'pop-up').style('position', 'absolute').style('opacity', 0).style('background', 'white');
    }
  }, {
    key: 'createSightLines',
    value: function createSightLines() {
      var m = this.margin,
          graph = d3.select('#graph');

      this.sightLine = graph.append('div').style('position', 'absolute').style('width', this.width - m.left - m.right - 15 + 'px').style('left', m.left + 'px').style('height', '1px').style('background-color', 'black').style('visibility', 'hidden');

      this.sightY = graph.append('div').style('position', 'absolute').style('width', 1 + 'px').style('height', this.height - m.bottom + 'px').style('bottom', m.bottom + 'px').style('background-color', 'black').style('visibility', 'hidden');
    }
  }, {
    key: 'createStack',
    value: function createStack() {
      this.stack = d3.stack().keys(['count']).order(d3.stackOrderNone).offset(d3.stackOffsetWiggle);
    }
  }, {
    key: 'createArea',
    value: function createArea() {
      ...
      this.area = d3.area().curve(d3.curveCatmullRom.alpha(0.5)).x(function (d, i) {
        return _this2.xScale(i);
      }).y0(function (d) {
        return _this2.yScale(d[0]);
      }).y1(function (d) {
    ...
    value: function createAxes() {
      ...
      var xAxisScale = d3.scaleLinear().domain([yrs[0], yrs[yrs.length - 1]]).range([m.left, this.width - m.right - 15]);

      this.xAxis = d3.axisBottom().scale(xAxisScale).ticks(10, "f").tickSizeOuter([0]);

      var max = d3.max(counts);
      if (max < StreamGraph.MIN_TOLERANCE) {
        max = 0;
      }
      var graphHeight = this.height - m.bottom - m.top;
      var yAxisScale = d3.scaleLinear().domain([0, max]).range([this.height / 2, 40]);

      this.yAxis = d3.axisLeft().scale(yAxisScale).ticks(10).tickSizeOuter([0]);
    }
  }, {
    key: 'createScales',
    value: function createScales() {
      var m = this.margin;
      var stackMax = function stackMax(layer) {
        return d3.max(layer, function (d) {
          return d[1];
        });
      };
      var stackMin = function stackMin(layer) {
        return d3.min(layer, function (d) {
          return d[0];
        });
      };

      this.xScale = d3.scaleLinear().domain([0, +this.data.length]).range([m.left, this.width - m.right]);

      var max = d3.max(this.layers, stackMax);
      var min = d3.min(this.layers, stackMin);
      if (max < StreamGraph.MIN_TOLERANCE) {
        max = 0;min = 0;
      }

      this.yScale = d3.scaleLinear().range([this.height - (m.top + m.bottom), 0]).domain([min, max]);
    }
  }], [{
    key: 'createPopUpHTML',
    value: function createPopUpHTML(_ref) {
      var year = _ref.year,
...
var Trie = function () {
  ...
  _createClass(Trie, null, [{
    key: "validate",
    value: function validate(node, value) {
      var match = node.children[value];
      return match || null;
    }
  }, {
    key: "insert",
...
module.exports = { Trie: Trie, TrieNode: TrieNode };
...
var removePunctuation = __webpack_require__(32);

var WordCounter = function () {
  function WordCounter(lyricsData) {
  ...
  _createClass(WordCounter, [{
    ...
    key: "toggleIncludeMax",
    value: function toggleIncludeMax() {
    ...
    value: function output() {
      ...
      Object.keys(this.current).forEach(function (yr) {
        var data = _this.current[yr];
        var count = data.count;
        if (!_this.includeMax && data.max) count -= data.max.count;
        if (_this.percent && data.total > 0) count /= data.total / 100;
    ...
    key: "expandTarget",
    value: function expandTarget(word) {
      var targets = [removePunctuation(word.toLowerCase())];
      if (word.slice(0, -3).search(/[aeiou]/) >= 0) {
        if (word.slice(-3) === "ing") targets.push(word.slice(0, -1));
      }
      return targets;
    }
  }, {
    ...
    value: function newCount() {
      ...
      var results = this.startResults();

      this.lyricsData.forEach(function (song) {
        var wordCounts = _this2.getWordCounts(song.Lyrics),
    ...
    key: "getWordCounts",
    value: function getWordCounts(words) {
      var _this3 = this;

      var total = 0,
          count = 0;

      Object.keys(words).forEach(function (wrd) {
        total += words[wrd];
        if (_this3.targets.includes(wrd)) count += words[wrd];
      });
      return [count, total];
    }
  }]);
...
function removePunctuation(string) {
  ...
  for (var i = 0; i < string.length; i++) {
    if (string[i] === " " || 65 <= string[i].charCodeAt() && string[i].charCodeAt() <= 122) newString += string[i];
  }
  return newString;
}
